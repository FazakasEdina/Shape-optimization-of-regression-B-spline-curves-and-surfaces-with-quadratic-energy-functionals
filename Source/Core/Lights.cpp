...
void DirectionalLight::setAmbientColor(const Color4& ambient)
{
    _ambient_intensity = ambient;
    glLightfv(_light_index, GL_AMBIENT,  &_ambient_intensity.r());
}
void DirectionalLight::setDiffuseColor(const Color4& diffuse)
{
    _diffuse_intensity = diffuse;
    glLightfv(_light_index, GL_DIFFUSE,  &_diffuse_intensity.r());
}
void DirectionalLight::setSpecularColor(const Color4& specular_intensity)
{
   _specular_intensity =  specular_intensity;
